...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, rel_pos_bias=None, return_attention=False, return_qkv=False):
        B, N, C = x.shape
        qkv_bias = None
        ...
        if mask is not None:
            nW = mask.shape[1]
            mask = mask.unsqueeze(1).unsqueeze(1).expand(B, attn.shape[1], nW, nW)
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, rel_pos_bias=None, return_attention=False, return_qkv=False):
        if return_attention:
            return self.attn(self.norm1(x), mask=mask, rel_pos_bias=rel_pos_bias, return_attention=True)
        if return_qkv:
            y, qkv = self.attn(self.norm1(x), mask=mask, rel_pos_bias=rel_pos_bias, return_qkv=return_qkv)
            x = x + self.drop_path(self.gamma_1 * y)
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
            return x, qkv

        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x), mask=mask, rel_pos_bias=rel_pos_bias))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), mask=mask, rel_pos_bias=rel_pos_bias))
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        return x
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
        ...
        if use_abs_pos_emb:
            self.pos_embed = nn.Parameter(torch.zeros(1, 12 + 1, embed_dim))
        else:
            self.pos_embed = None
        self.time_embed = nn.Parameter(torch.zeros(1, 20, embed_dim), requires_grad=True)
        self.abs_time_embed = nn.Embedding(1000, embed_dim)
    ...
    def forward_features(self, x, mask=None, t_indices=None, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        batch_size, n, a, t = x.shape
        input_time_window = a if t == self.patch_size else t
        x = self.patch_embed(x)
        mask = rearrange(mask, 'B N A -> B (N A)') if mask is not None else None
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks

        x = torch.cat((cls_tokens, x), dim=1)
        mask = torch.cat((torch.ones((batch_size, 1), device=x.device, dtype=torch.bool), mask), dim=1) if mask is not None else None
        ...
        if t_indices is not None:
            t_indices = rearrange(t_indices, 'B N A -> B (N A)')
            time_embed = self.abs_time_embed(t_indices)
            x[:, 1:, :] += time_embed
        else:
            if self.time_embed is not None:
        ...
        for blk in self.blocks:
            x = blk(x, mask=mask, rel_pos_bias=None)
    ...
    def forward(self, x, mask=None, t_indices=None, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        '''
        ...
        '''
        x = self.forward_features(x, mask=mask, t_indices=t_indices, input_chans=input_chans, 
...
def labram_base_patch200_200(pretrained=False, **kwargs):
    model = NeuralTransformer(
        patch_size=96, embed_dim=96, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
