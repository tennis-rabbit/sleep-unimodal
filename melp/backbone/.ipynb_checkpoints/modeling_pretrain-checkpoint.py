# --------------------------------------------------------
# https://github.com/935963004/LaBraM/blob/main/modeling_pretrain.py 
...
class NeuralTransformerForMaskedEEGModeling(nn.Module):
    ...
    def forward_features(self, x, mask=None, t_indices=None, input_chans=None, bool_masked_pos=None):
        ...
        mask = torch.cat([torch.ones((batch_size, 1), dtype=torch.bool).to(x.device), mask], dim=1) if mask is not None else None
        ...
        for blk in self.blocks:
            x = blk(x, mask=mask, rel_pos_bias=rel_pos_bias)

        return self.norm(x)

    def forward(self, x, mask=None, t_indices=None, input_chans=None, bool_masked_pos=None, return_all_tokens=False, 
        ...
        x = self.forward_features(x, mask=mask, t_indices=t_indices, bool_masked_pos=bool_masked_pos)
...
class NeuralTransformerForMEM(nn.Module):
    ...
    def forward(self, x, mask=None, t_indices=None, input_chans=None, bool_masked_pos=None):
        x_masked = self.student(x, mask, t_indices, input_chans, bool_masked_pos, return_all_patch_tokens=True)
        x_masked_no_cls = x_masked[:, 1:]
        x_rec = self.lm_head(x_masked_no_cls[bool_masked_pos])

        #symetric
        x_masked_sym = self.student(x, mask, t_indices, input_chans, ~bool_masked_pos, return_all_patch_tokens=True)
...
def labram_base_patch200_1600_8k_vocab(pretrained=False, **kwargs): #5M
    ...
    model = NeuralTransformerForMEM(
        patch_size=96, embed_dim=96, depth=12, num_heads=10, mlp_ratio=4, qkv_bias=False, qk_norm=partial(nn.LayerNorm, eps=1e-6),
