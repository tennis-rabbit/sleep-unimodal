...
class TransformerModel(PretrainingModel):
    def __init__(self, cfg: TransformerConfig):
        ...
        if cfg.apply_mask:
            # self.mask_emb = nn.Parameter(torch.Tensor(cfg.encoder_embed_dim).uniform_())
            self.mask_emb = nn.Parameter(
                torch.zeros(cfg.encoder_embed_dim)
            )
            nn.init.normal_(self.mask_emb, mean=0, std=cfg.encoder_embed_dim ** -0.5)
    ...
    def apply_mask(
        ...
        if self.mask_prob > 0:
            if mask_indices is None:
                ...
                mask_indices = torch.from_numpy(mask_indices).type_as(x).bool()
