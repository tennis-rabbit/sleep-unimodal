...
class VQNSP(nn.Module):
    ...
    def get_tokens(self, data, mask, t_indices=None, input_chans=None, **kwargs):
        # data: [B, N, A, T]
        # t_indices: [B, N, A]
        quantize, embed_ind, loss = self.encode(data, mask=mask, t_indices=t_indices, input_chans=input_chans)
        output = {}
    ...
    def encode(self, x, mask=None, t_indices=None,  input_chans=None):
        batch_size, n, a, t = x.shape
        encoder_features = self.encoder(x, mask=mask, t_indices=t_indices, input_chans=input_chans, return_patch_tokens=True)
        ...
        quantize, loss, embed_ind = self.quantize(to_quantizer_features, mask=mask)

        return quantize, embed_ind, loss
    
    def decode(self, quantize, mask=None, t_indices=None, input_chans=None, **kwargs):
        ...
        decoder_features = self.decoder(quantize, mask, t_indices, input_chans, return_patch_tokens=True)
        rec = self.decode_task_layer(decoder_features)
        rec_angle = self.decode_task_layer_angle(decoder_features)
        return rec, rec_angle
    
    def get_codebook_indices(self, x, mask=None, t_indices=None, input_chans=None, **kwargs):
        # for LaBraM pre-training
        return self.get_tokens(x, mask, t_indices, input_chans, **kwargs)['token']
    ...
    def forward(self, x, t_indices, input_chans=None, **kwargs):
        """
        x: shape [B, N, T]
        """
        ...
        quantize, embed_ind, emb_loss = self.encode(x, mask, t_indices, input_chans)
        xrec, xrec_angle = self.decode(quantize, mask, t_indices, input_chans)
        amplitude = rearrange(amplitude, 'b n a c -> b (n a) c')
        angle = rearrange(angle, 'b n a c -> b (n a) c')
        flatten_mask = rearrange(mask, 'b n a -> b (n a)')
        rec_loss = self.calculate_rec_loss(xrec[flatten_mask], amplitude[flatten_mask])
        rec_angle_loss = self.calculate_rec_loss(xrec_angle[flatten_mask], angle[flatten_mask])
        loss = emb_loss + rec_loss + rec_angle_loss
        
        loss_dict = {}
        loss_dict[f'quant_loss'] = emb_loss
        loss_dict[f'rec_loss'] = rec_loss
        loss_dict[f'rec_angle_loss'] = rec_angle_loss
        loss_dict[f'loss'] = loss

        return loss_dict

def get_model_default_params():
    return dict(EEG_size=960, patch_size=96, in_chans=1, num_classes=1000, embed_dim=96, depth=12, num_heads=10,  
...
def vqnsp_encoder_large_decoder_3x200x24(pretrained=False, pretrained_weight=None, as_tokenzer=False, EEG_size=1600, 
                                         n_code=8192, code_dim=32, patch_size=96, **kwargs):

    encoder_config, decoder_config = get_model_default_params(), get_model_default_params()
    encoder_config["patch_size"] = patch_size
    encoder_config["embed_dim"] = encoder_config["patch_size"]
    decoder_config["patch_size"] = patch_size
    decoder_config["embed_dim"] = decoder_config["patch_size"]
