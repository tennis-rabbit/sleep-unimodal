...
#         return {'loss': loss_recon, 'loss_recon': loss_recon}, metrics_dict
    def shared_step(self, batch, batch_idx):
        ...
        bz, c, t = sig.shape
        sig = sig.view(bz, c, 10, 3840)    # -> (bz, 2, 10, 3840)
        sig = sig.permute(0, 2, 1, 3)      # -> (bz, 10, 2, 3840)
        sig = sig.reshape(bz * 10, c, 3840)  # -> (bz*10, 2, 3840)
        ############################
        ################## replace the encode for MAE
        ################## replace the encode for MAE
        enc = self.encoders['all']                      # PSGModalityEncoder
        vit = enc.backbone                              # ViT-1D
        # a. patchify
        #print("111:",sig.shape)
        x = vit.to_patch_embedding(sig)                 # (B, width, N_patch)
        #print("222:",x.shape)
        x = rearrange(x, 'b c n -> b n c')              # (B, N, width)

        # b. add pos & random mask
        x = x + vit.pos_embedding      
        
        x_vis, mask, ids_restore = self._random_mask(x) # mask: 1=mask, 0=keep

        # c. encode visible tokens
        for i in range(vit.depth):
            x_vis = getattr(vit, f'block{i}')(x_vis)
        latent = vit.norm(x_vis)                        # (B, N_keep, width)
        
        latent = enc.proj_head(latent) # to the contrastive latent space, just for consistency
        ...
        mask_tokens = self.mask_token.repeat(B, mask.size(1) - N_vis, 1)
        ...
        pred = self.pred_head(x_full)                   # (B, N, patch_pixels)
        
        
        # e. target & loss
        target = rearrange(sig, 'b c (n p) -> b n (c p)', p=enc.patch_size)     # (B, N, patch_pixels)
        
        
        
        mse = ((pred - target) ** 2).mean(dim=-1)       # (B, N)
