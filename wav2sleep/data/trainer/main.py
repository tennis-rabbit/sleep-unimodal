...
class SleepLightningModel(lightning.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx: int = 0):
        ...
        if ECG in batch[0] and ECG in self.model.signal_encoders.signals:
            self._step(batch, mode=VAL, dataloader_idx=dataloader_idx, signals=(ECG,))
            # Validate ECG/THX on SHHS, MESA
            if THX in batch[0] and THX in self.model.signal_encoders.signals and ds_name in (SHHS, MESA):
                self._step(batch, mode=VAL, dataloader_idx=dataloader_idx, signals=(ECG, THX))
        # Validate on PPG for datasets where available.
        if PPG in batch[0] and PPG in self.model.signal_encoders.signals and ds_name in (MESA, CFS, CCSHS, CHAT):
            self._step(batch, mode=VAL, dataloader_idx=dataloader_idx, signals=(PPG,))
            if THX in batch[0] and THX in self.model.signal_encoders.signals and ds_name in (MESA,):
    ...
    def test_step(self, batch, batch_idx, dataloader_idx: int = 0):
        ...
        if ECG in batch[0] and ECG in self.model.signal_encoders.signals:
            ...
            if THX in batch[0] and THX in self.model.signal_encoders.signals:
